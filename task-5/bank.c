...
int *parent_time; 						//wait time of parent
int *child_1_time; 						//wait time of child 1
int *child_2_time; 						//wait time of child 2
main()
{
	...
	int parent, child_1, child_2;   // pid
	int semaphore; 					// semaphore declaration
	
	//location of process pointers
	int a = 0;
	int b = 0;
	int c = 0;
	parent_time = &a;
	...
	//First Child Process. Dear old dad tries to do some updates.
		...
		for(i=1;i<=N; i++)
		{
			...
			V(semaphore); 		//exited crtial section
		}
	}
	...
	else
	{
		...
		if (pid == CHILD)
		{
			...
			{
				child_1 = getpid();
				P(semaphore); 		//entered critical section
				if (parent != 0)
				{
					*parent_time = a + 1;
				}
		...
		//Parent Process. Fork off one more child process.
			...
			if (pid == CHILD)
			{
				...
				while(flag1 == FALSE) 
				{
					child_2 = getpid();  	//saving child 2 pid
					P(semaphore); 			//enetered crticial section
					if(parent != 0)
					{
						*parent_time = a + 1;
					}
					...
					V(semaphore);  			//exiting critical section.
				}
			}
			else
			{
		...
		//display wait time of pid's
		printf("Time of parent: %d\n",*parent_time);
		printf("Time of child 1: %d\n",*child_1_time);
		printf("Time of child 2: %d\n",*child_2_time);
		exit(0);
	}
	exit(0);
}
